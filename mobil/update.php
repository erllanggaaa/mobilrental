...
if(isset($_POST['ubah_foto'])){ // Jika menceklis checkbox yang ada di form ubah, lakukan :
	...
	if(move_uploaded_file($nama_gambar, $path)){ // Cek apakah gambar berhasil diupload atau tidak
		...
		$query = "SELECT tb_mobil.id_mobil, tb_mobil.foto_mobil, tb_jenis.nama_jenis, tb_mobil.type_mobil, tb_mobil.merk, tb_mobil.no_polisi, tb_mobil.warna, tb_mobil.harga, tb_mobil.status from mobilrental.tb_jenis inner join mobilrental.tb_mobil on (tb_jenis.id_jenis = tb_mobil.id_jenis) where id_mobil";
		$sql = mysql_query($query);
		$data = mysql_fetch_array($sql);

		// Cek apakah file gambar sebelumnya ada di folder gambar
		if(is_file("gambar/".$data['foto_mobil'])) // Jika gambar ada
			unlink("gambar/".$data['foto_mobil']); // Hapus file gambar sebelumnya yang ada di folder images
